...
export const activate = (context: vscode.ExtensionContext): void => {
    const getConfig = () =>
        vscode.workspace.getConfiguration("visibleWhitespace");

    const isLanguageEnabled = (languageId: string): boolean => {
        const enabledLanguages = getConfig().get<string[]>(
            "enabledLanguageIds",
            [],
        );
        return (
            enabledLanguages.length === 0 ||
            enabledLanguages.includes(languageId)
        );
    };
    ...
    const createResources = (): void => {
        ...
        const overlayColor = config.get<string>(
            "overlayColor",
            "rgba(128,128,128,1)",
        );
        ...
        if (config.get<boolean>("newLine.enable", true)) {
            const newLineColor = config.get<string>(
                "newLine.color",
                "rgba(128,128,128,1)",
            );
            // for New Line (U+000A)
    ...
    const updateDecorations = (editor: vscode.TextEditor): void => {
        ...
        options.forEach((decoOptions, decoType) =>
